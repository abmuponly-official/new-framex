    ...
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <div>
              <h2 className="text-display-sm font-semibold text-brand-black mb-8">
                {locale === 'vi' ? 'Những rủi ro quen thuộc' : 'Familiar risks'}
              </h2>
              <ul className="space-y-4">
                {[t('pain_1'), t('pain_2'), t('pain_3'), t('pain_4')].map((pain, i) => (
                  <li key={i} className="flex items-start gap-4 p-5 border border-brand-gray-100 rounded-sm">
                    ...
                    <span className="text-brand-gray-700">{pain}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              <h2 className="text-display-sm font-semibold text-brand-black mb-8">
                {locale === 'vi' ? 'FrameX giải quyết thế nào' : 'How FrameX solves it'}
              </h2>
              <ul className="space-y-4">
      ...
      <section className="py-20 bg-brand-gray-50 border-t border-brand-gray-100">
        <div className="container-base text-center max-w-xl">
          <Link
            href={`/${locale}/lien-he?role=investor`}
